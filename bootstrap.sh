...
zpool create -f -o ashift=12 -o cachefile=/tmp/zpool.cache -O compression=lz4 -O atime=off -O dedup=on -O sync=disabled -m none -R /mnt rpool mirror $ZFS_DEV1 $ZFS_DEV2
...
chroot /mnt ./bootstrap_chroot.sh $BOOT_DEV
